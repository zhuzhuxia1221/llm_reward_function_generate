...
def main(args):
    ...
    result_columns = ["user", "ilp_user", "ilp_time", "greedy_user", "greedy_time", "rl_user", "rl_time"]
    ...
    for U in range(400, 800, 100):
        for epoch in range(args.episodes):
            ...
            greedy_time = timeit.default_timer() - start

            start = timeit.default_timer()
            rl_alloc = rl_algo(U, args.N, args.S, ngb, service, server_state, model_rl)
            rl_time = timeit.default_timer() - start

            result_df.loc[len(result_df)] = [U, len(ilp_alloc), ilp_time, len(greedy_alloc), greedy_time, len(rl_alloc), rl_time]
            print(f"[Users={U}] ILP={len(ilp_alloc)}, Greedy={len(greedy_alloc)}, RL_Und={len(rl_alloc)}")

    result_df.to_csv(args.output, index=False)
    print(f"Results saved to {args.output}")

    plt.figure(figsize=(10, 6))
    for algo in ["ilp_user", "greedy_user", "rl_user"]:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_path", type=str, default="saved_models/AAnew_edge_agent_threats_und.zip")
